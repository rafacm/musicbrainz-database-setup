...
- `admin/sql/CreateCollations.sql` requires the operator-installed C extensions `musicbrainz_collate` and `musicbrainz_unaccent`. `schema/extensions.preflight()` checks `pg_available_extensions` and aborts with a pointer to `docs/PREREQUISITES.md` rather than letting the raw SQL fail cryptically.
