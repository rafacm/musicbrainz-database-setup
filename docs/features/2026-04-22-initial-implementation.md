...
Bootstrapped the repository with a `uv`-managed Python 3.12 package (`src/musicbrainz_db_setup/`) plus Typer CLI (`musicbrainz-db-setup`). The pipeline is split into four subpackages that `cli.run` orchestrates in order:
...
| `schema/` | Ordered DDL runner matching `admin/InitDb.pl` (Extensions, Collations, Types, Tables → COPY → PrimaryKeys, Functions, Indexes, FKs, Constraints, Sequences, Views, Triggers). Pre-flights `pg_available_extensions`. Records applied phases in `musicbrainz_db_setup.applied_phases`. |
| `importer/` | `tarfile.open(mode="r\|bz2")` streaming; each `mbdump/<table>` member piped into `COPY <schema>.<table> FROM STDIN` via psycopg3's `cursor.copy()`. One transaction per archive. Bookkeeping in `musicbrainz_db_setup.imported_archives`. |
...
| 2 | `uv run musicbrainz-db-setup --help` | Prints the 7-subcommand tree. |
| 3 | `uv run pytest -q` | 20 tests pass. |
| 4 | `uv run ruff check src tests` | Clean. |
| 5 | `uv run musicbrainz-db-setup list-dumps --limit 5` | Returns real dated dirs from `data.metabrainz.org`. |
...
| `src/musicbrainz_db_setup/__init__.py`, `__main__.py` | Package + `python -m` entry. |
| `src/musicbrainz_db_setup/cli.py` | Typer app: `list-dumps`, `download`, `schema create`, `import`, `run`, `verify`, `clean`. |
| `src/musicbrainz_db_setup/config.py` | `Settings` class (pydantic-settings) with `MUSICBRAINZ_DB_SETUP_*` env vars. |
| `src/musicbrainz_db_setup/errors.py` | `MBSetupError` hierarchy + `ExitCode` constants. |
| `src/musicbrainz_db_setup/logging.py` | Rich console + optional file handler. |
| `src/musicbrainz_db_setup/progress.py` | `ProgressManager` singleton wrapping `rich.progress.Progress`. |
| `src/musicbrainz_db_setup/db.py` | psycopg3 `connect()` + `bulk_session()` context manager. |
| `src/musicbrainz_db_setup/verify.py` | Read `SCHEMA_SEQUENCE` / `REPLICATION_SEQUENCE` from archives. |
| `src/musicbrainz_db_setup/mirror/{client,index,checksums,download}.py` | httpx wrapper, dated-dir listing, SHA256/MD5 parse + verify, resumable download. |
| `src/musicbrainz_db_setup/sql/{github,cache,manifest}.py` | GitHub ref→SHA resolver, raw fetch, SHA-keyed cache, per-module SQL file lists. |
